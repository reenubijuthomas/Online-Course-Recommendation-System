...
def recommendation(request):
    ...
    for user in users:
        if user != request.user:
            ...
            other_liked_courses[user.pk] = Course.objects.filter(
                pk__in=other_likes[user.pk].values_list('course', flat=True))
            other_disliked_courses[user.pk] = Course.objects.filter(
                pk__in=other_dislikes[user.pk].values_list('course', flat=True))
            ...
            lst = [user_liked_courses, user_disliked_courses, other_liked_courses[user.pk],
                   other_disliked_courses[user.pk]]
            total = list(set().union(*lst))

            if len(total) != 0:
                similarity[user.pk] = (len(common_likes) + len(common_dislikes) - len(opposite_rating1) - len(
                    opposite_rating2)) / len(total)
            else:
    ...
    return render(request, "recommendation.html",
                  {"hybrid": courses_hybrid_final, "content": courses_content_final, "collab": courses_collab_final})
